...
if [[ -z "$WEB_SG_ID" || "$WEB_SG_ID" == "None" ]]; then
  ...
  aws ec2 authorize-security-group-ingress --region "$AWS_REGION" --profile "$AWS_PROFILE" \
    --group-id "$WEB_SG_ID" --ip-permissions "IpProtocol=tcp,FromPort=80,ToPort=80,IpRanges=[{CidrIp=0.0.0.0/0,Description=HTTP}]"
  aws ec2 authorize-security-group-ingress --region "$AWS_REGION" --profile "$AWS_PROFILE" \
    --group-id "$WEB_SG_ID" --ip-permissions "IpProtocol=tcp,FromPort=22,ToPort=22,IpRanges=[{CidrIp=${MY_IP_CIDR},Description=SSH}]"
...
# ===== User-Data (Apache/PHP + Mini-App) =====
DB_PASS_ESC=${DB_PASS//\'/\'\"\'\"\'}
read -r -d '' UD <<EOF
#!/bin/bash
set -euxo pipefail
dnf -y update
dnf -y install httpd php php-pgsql php-cli curl

echo "upload_max_filesize=${UPLOAD_MAX_MB}M" > /etc/php.d/99-smartfit.ini
echo "post_max_size=$((UPLOAD_MAX_MB+2))M" >> /etc/php.d/99-smartfit.ini
...
SMARTFIT_DB_HOST=${DB_PRIVATE_IP}
SMARTFIT_DB_NAME=${DB_NAME}
SMARTFIT_DB_USER=${DB_USER}
SMARTFIT_DB_PASS='${DB_PASS_ESC}'
SMARTFIT_UPLOAD_MAX_MB=${UPLOAD_MAX_MB}
ENV
echo "EnvironmentFile=/etc/sysconfig/httpd-smartfit" >> /usr/lib/systemd/system/httpd.service
...
try { $pdo = new PDO("pgsql:host=$host;port=5432;dbname=$db",$user,$pass,[PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION,PDO::ATTR_DEFAULT_FETCH_MODE=>PDO::FETCH_ASSOC]); }
catch (Throwable $e) { http_response_code(500); echo "DB-Fehler: ".htmlspecialchars($e->getMessage(),ENT_QUOTES,'UTF-8'); exit; }
PHP
  ...
  if (!isset($allowed[$mime])) $msg="Nur JPEG/PNG/WEBP";
    ...
    if (!in_array($ext, $allowed[$mime], true)) $msg="Endung passt nicht zum MIME";
      ...
      if (move_uploaded_file($f['tmp_name'], __DIR__."/uploads/".$stored)) {
        $pdo->prepare("INSERT INTO items (user_id,image_path,created_at,updated_at) VALUES (NULL,:p,NOW(),NOW())")->execute([':p'=>"/uploads/".$stored]);
        $msg="Upload ok";
